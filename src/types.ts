...
export type FactorKey = 'hospital' | 'airport' | 'commute' | 'crime' | 'space';
 ...
 * `crime` is optional so sites saved before it existed default to a neutral
 * score until scored. Older states may also carry a retired `risk` key.
 */
export type Scores = {
  hospital: number;
  airport: number;
